 ...
 * Multer already parsed:
 *  - req.file  -> uploaded resume file (if sent)
 *  - req.body  -> all textual fields (strings)
 * Auth middleware (authRequired) must set req.user = { id, email, role }.
 */
export const createApplication = async (req, res) => {
  try {
    // require authenticated user (no more fallback to 1)
    if (!req.user?.id) {
    ...
    // Shallow copy and normalize a few fields so DB insert won’t choke
    const payload = { ...form };
    ...
    // skills may arrive as JSON string or a single value
    ...
    if (payload.availability_for_interview === "")
      payload.availability_for_interview = null;
    ...
    // If Multer saved a file, prefer its stored filename as resume_path
    if (file?.filename) {
      payload.resume_path = file.filename;
    }

    // Hand off to service (no server-side mapping needed)
